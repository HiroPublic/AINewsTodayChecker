...
GitHub Actions を正式な起動トリガーにした、Python 3.12 ベースの AI ニュース検証 MVP です。ポッドキャスト「最新AI情報 AIニューストゥデイ」の RSS feed から最新回を取得し、概要欄から claim を抽出し、ルールベースで真偽・誇張・未確認を判定して Slack Incoming Webhook に通知します。

## 特徴

- GitHub Actions の `schedule` と `workflow_dispatch` を正式トリガーとして利用
- `.state/last_run.json` を `actions/cache` で保持し、`content_hash` ベースで重複通知を防止
...
- 判定ロジックはコード主導のルールベース。OpenAI は将来拡張用 placeholder のみ
...
`SLACK_WEBHOOK_URL` が未設定の場合、通知処理は失敗し exit code 1 になります。MVP の実通知確認時は `.env` か環境変数で設定してください。
