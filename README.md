...
GitHub Actions を正式な起動トリガーにした、Python 3.12 ベースの AI ニュース検証アプリです。ポッドキャスト「最新AI情報 AIニューストゥデイ」の RSS feed から最新回を取得し、概要欄を LLM API で評価し、結果を Slack Incoming Webhook に通知します。
...
- 既定では Gemini API を利用し、フラグで Perplexity API に切り替え可能
- API 未設定や失敗時はルールベース判定にフォールバック
...
`SLACK_WEBHOOK_URL` が未設定の場合、通知処理は失敗し exit code 1 になります。Perplexity API を使う場合は `PERPLEXITY_API_KEY`、Gemini API を使う場合は `GEMINI_API_KEY` を設定してください。未設定時はルールベースで継続します。
`PODCAST_RSS_URL` はデフォルトで `https://podcasts.loudandfound.com/ainewstoday/podcast.xml` を参照します。
実行時だけ切り替えたい場合は `--provider gemini` または `--provider perplexity` を指定できます。
...
- `artifacts/prompt.json` と `artifacts/perplexity-response.json` を workflow artifact として保存
- `artifacts/` に差分があれば repository に commit / push
...
6. `artifacts/prompt.json` と `artifacts/perplexity-response.json` を workflow artifact として保存し、差分があれば branch へ push
7. 通知成功を含む job success 時のみ `.state` cache を save
...
- `VERIFIER_PROVIDER` の既定値は `gemini` です
- `VERIFIER_PROVIDER=perplexity` を設定すると Perplexity API を使います
- `PERPLEXITY_MODEL` の既定値は `sonar` です
- `GEMINI_MODEL` の既定値は 'gemini-3-flash-preview` です
...
```bash
python scripts/run_daily.py --preview --provider gemini
```
